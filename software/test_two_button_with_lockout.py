...
"""Tests for two_button_with_lockout.py"""
...
import two_button_with_lockout
...
class SimpleDispatcherTest(unittest.TestCase):
  def setUp(self):
    ...
    self.dispatcher = two_button_with_lockout.Dispatcher(config)
  ...
  def test_auth_flow(self):
    ...
    self.dispatcher.abort(None)
    self.dispatcher.badge_scan('1234')
    self.assertTrue(self.dispatcher.locked)
    self.dispatcher.off_button_released(None)
    # Locked state
    self.dispatcher.badge_scan('2345')
    self.assertFalse(self.dispatcher.authorized)
    self.dispatcher.on_button_down(None)
    self.assertFalse(GPIO.input(5))
    # Unlocked state same badge
    self.dispatcher.badge_scan('1234')
    self.assertTrue(self.dispatcher.authorized)
    self.assertFalse(GPIO.input(5))
    self.dispatcher.on_button_down(None)
    ...
    self.dispatcher.abort(None)
    self.dispatcher.badge_scan('1234')
    self.assertFalse(self.dispatcher.locked)
    self.dispatcher.off_button_released(None)
    # Unlocked state check with different badge
    self.dispatcher.badge_scan('2345')
    self.assertTrue(self.dispatcher.authorized)
    self.assertFalse(GPIO.input(5))
    self.dispatcher.on_button_down(None)
